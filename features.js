...
ipcMain.on("apply-user-windows-features", (event, selectedIds) => {
  ...
  const commands = userRegistryChanges
    .map((opt) => {
      if (opt.command) {
...
ipcMain.on("apply-machine-windows-features", (event, selectedIds) => {
  ...
  const commands = machineRegistryChanges
    .map((opt) => {
      if (opt.command) {
        return selectedIds.includes(opt.id) ? wrapCommand(opt.command) : null;
      }
  ...
  const results = await Promise.all(
    commands.map(async (command) => {
      ...
      return new Promise((resolve) => {
        ...
        psProcess.on("close", () => {
          const valuePattern = valueType === "REG_DWORD" ? `${valueName}\\s+${valueType}\\s+0x${Number.parseInt(expectedValue).toString(16)}` : `${valueName}\\s+${valueType}\\s+${expectedValue}`;
